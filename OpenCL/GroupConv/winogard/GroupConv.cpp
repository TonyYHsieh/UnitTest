...
void fill_const(float* buf, size_t size, float value)
{
    for(size_t i=0; i<size; i++)
    {
        buf[i] = value;
...
int main(int argc, char* argv[])
{
    ...
    // build progream
    std::string option;
    option = option + " -DK_PER_WORK=" + std::to_string(k_per_work)
                    + " -DGROUP_SIZE=" + std::to_string(group_size);
    err = clBuildProgram(program, 1, &device, option.c_str(), NULL, NULL);
    if (err != CL_SUCCESS)
    {
        char buffer[10240];
    ...
    // build kernels
    cl_kernel kernel_wino = clCreateKernel(program, "WinoWeightConvert" , &err);
    ...
    size_t wino_weight_size = config.m_k * config.m_c * (config.m_y + 1) * (config.m_x + 1) / config.m_g;
    ...
    srand(time(NULL));
//    fill_const(inputbuf_h, input_size, 1.0f);
//    fill_const(weightbuf_h, weight_size, 1.0f);
    ...
    checkCLError(err, "clCreateBuffer");
    cl_mem wino_weightbuf = clCreateBuffer(context, CL_MEM_READ_WRITE, sizeof(float) * wino_weight_size, NULL, &err);
    checkCLError(err, "clCreateBuffer");
    ...
    for(int i=0; i<TEST_ITERS; i++)
    {
        ...
        {
            cl_event event;
            int gkc = config.m_k * config.m_c / config.m_g;
            int works = ((gkc + 255) / 256) * 256;

            size_t global_work_size[3] = {works, 1, 1};
            size_t local_work_size[3] = {256, 1, 1};

            checkCLError(clSetKernelArg(kernel_wino, 0, sizeof(cl_mem), &weightbuf), "clSetKernelArg");
            checkCLError(clSetKernelArg(kernel_wino, 1, sizeof(cl_mem), &wino_weightbuf), "clSetKernelArg");
            checkCLError(clSetKernelArg(kernel_wino, 2, sizeof(cl_int), &gkc), "clSetKernelArg");

            err = clEnqueueNDRangeKernel(commandQueue, kernel_wino, 3, NULL, global_work_size, local_work_size, 0, NULL, &event);
...
#if 1
        {
            ...
            size_t global_work_size[3] = {config.m_w / 2, config.m_h / 2, config.m_k * config.m_n / k_per_work};
            size_t local_work_size[3] = {group_size, group_size, 1};

            checkCLError(clSetKernelArg(kernel, 0, sizeof(cl_mem), &inputbuf), "clSetKernelArg");
            checkCLError(clSetKernelArg(kernel, 1, sizeof(cl_mem), &wino_weightbuf), "clSetKernelArg");
    ...
    for(int n=0; n<config.m_n; n++)
    {
        for(int k=0; k<config.m_k; k++)
        {
            for(int j=0; j<config.m_h; j++)
            {
                ...
                {
                    std::cout << outputbuf_d[((n * config.m_k + k) * config.m_h + j) * config.m_w + i] << " ";
                }
                std::cout << std::endl;
            }
        }
    ...
    for(int n=0; n<config.m_n; n++)
    {
        for(int k=0; k<config.m_k; k++)
        {
            for(int j=0; j<config.m_h; j++)
            {
                for(int i=0; i<config.m_w; i++)
                {
                    std::cout << outputbuf_h[((n * config.m_k + k) * config.m_h + j) * config.m_w + i] << " ";
                }
                std::cout << std::endl;
            }
        }
