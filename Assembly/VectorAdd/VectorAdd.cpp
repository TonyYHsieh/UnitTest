        ...
        cl_mem& output)
{
    ...
    checkCLError(clSetKernelArg(kernel, 2, sizeof(cl_mem), &output), "clSetKernelArg");
...
int main()
{
    ...
    // create buffer
    cl_mem inputa = clCreateBuffer(context, CL_MEM_READ_ONLY,  sizeof(int)*NUM_GROUPS*GROUP_SIZE, NULL, &err);
    cl_mem inputb = clCreateBuffer(context, CL_MEM_READ_ONLY,  sizeof(int)*NUM_GROUPS*GROUP_SIZE, NULL, &err);
    ...
    // fillbuffer
    int value = 4;
    checkCLError(clEnqueueFillBuffer(commandQueue, inputa, &value, sizeof(value), 0, sizeof(int)*NUM_GROUPS*GROUP_SIZE, 0, nullptr, nullptr), "clEnqueueFillBuffer");
    value = 5;
    checkCLError(clEnqueueFillBuffer(commandQueue, inputb, &value, sizeof(value), 0, sizeof(int)*NUM_GROUPS*GROUP_SIZE, 0, nullptr, nullptr), "clEnqueueFillBuffer");
    ...
    {
        launchKernel(commandQueue, kernel, inputa, inputb, output);
    }
